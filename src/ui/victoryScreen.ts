...
function showQuizInvitationModal(
    k: KaboomCtx,
    gameState: GameState,
    onFinish: () => void
) {
    ...
    // Subtítulo
    elements.push(k.add([
        k.text("Você guiou a baleia ao Santuário de Arraial do Cabo!", { size: 12, font: "sans-serif" }),
...
function renderVictoryContent(k: KaboomCtx, gameState: GameState, onRestart: () => void) {
    ...
    const buildStatsText = () => {
        ...
        const breachText = gameState.hasBreached() ? "✨ Salto Majestoso (Breach): EXECUTADO (+500 pts)\n" : "";
        const quizText = gameState.getQuizScore() > 0 
    ...
    const quizBtnLabel = k.add([
        k.text(quizCompleted ? `Quiz Concluído! (+${gameState.getQuizScore()} pts) ✓` : "Fazer Quiz [ENTER] (+300 pts) ▶", {
            size: 11.5,
            font: "sans-serif",
        }),
        k.pos(quizPanelX, quizPanelY + 60),
